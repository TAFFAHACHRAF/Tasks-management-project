...
public class ProjetDmpl implements ProjetDAO {
        ...
        try {
            ...
            while (rs.next()){
                ...
                if(rs4.next()) {
                    ...
                    responsable.setCOMPTE(new Compte( rs.getLong("USER_ID"),rs4.getString("EMAIL") , rs4.getString("PASSWORD")));
    ...
    public Projet save(Projet p) throws SQLException {
        try {
            Connection connection=new SingletonConnexionDB().getConnexion();
            PreparedStatement stmt = connection.prepareStatement("INSERT INTO projets (ID, TITLE, DATE_DEBUT, USER_ID) VALUES (?, ?, ?, ?, ?, ?)");
            stmt.setLong(1, p.getID());
            stmt.setString(2, p.getTITLE());
            stmt.setDate(3, (Date) p.getDate());
            stmt.setLong(4, p.getRESPONSABLE().getID());
            stmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Projet update(Projet p) throws SQLException {
        try {
            Connection connection=new SingletonConnexionDB().getConnexion();
            PreparedStatement stmt  = connection.prepareStatement("UPDATE projets SET TITLE = ?, DATE_DEBUT = ?, USER_ID=? WHERE ID = ?");
            stmt.setString(1, p.getTITLE());
            stmt.setDate(2, (Date) p.getDate());
            stmt.setLong(3, p.getRESPONSABLE().getID());
